...
public class ImageMirroringY extends BMPMirroring {

    public ImageMirroringY(String path) throws IOException {
        super(path);
    }
    ...
    public void MirrorImage(String path) throws IOException {
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                ...
                int end = startingAddr + (height - i - 1) * width * (bits / 8) + j * (bits / 8) - (bits / 8);
